  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        <h1 className="font-display text-4xl font-bold text-white mb-2">O'yinchilar kutmoqda</h1>
        <p className="text-white/40 text-lg">{session.quiz.title}</p>
        <p className="text-white/30 text-sm mt-1">
          {session.quiz.questions.length} ta savol •
          Umumiy vaqt: {Math.floor(session.totalTime / 60)}:{(session.totalTime % 60).toString().padStart(2,'0')}
        </p>
      </div>
      ...
      <div className="bg-gradient-card rounded-2xl p-6 text-center glow-purple">
        <p className="text-white/40 text-sm font-display uppercase tracking-widest mb-2">Sessiya</p>
        <p className="font-display text-xl font-bold text-purple-300 select-all">{session.id}</p>
      </div>

      <div>
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-display text-xl font-bold text-white">
            Ulangan o'yinchilar <span className="text-purple-400">{players.length}</span>
          </h2>
        </div>
        ...
        {players.length === 0 ? (
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-3">
            {players.map(player => (
              <div key={player.uid} className="bg-gradient-card rounded-2xl p-3 text-center animate-slide-up">
      ...
      <div className="flex gap-4 justify-center">
        <button onClick={onEnd} className="px-6 py-3 bg-white/5 hover:bg-white/10 text-white/60 font-display rounded-xl transition-all">
          Bekor qilish
        </button>
        <button
          onClick={onStart}
        ...
        >
          {isCountdown ? '🚀 Boshlanmoqda...' : `▶ Boshlash (${players.length} o'yinchi)`}
        </button>
      </div>
    </div>
  )
