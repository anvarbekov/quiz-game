...
import {
  ...
  startCountdown, revealAnswer, nextQuestion, showLeaderboard, endSession,
  subscribeSession, getActiveSessionId
...
export default function AdminPage() {
  ...
  async function handleSaveQuiz(quiz: Quiz) {
    await saveQuiz(quiz)
    const updated = await getQuizzes()
    setQuizzes(updated)
    setView('dashboard')
    setEditingQuiz(null)
  }
      ...
      <main className="p-6 max-w-7xl mx-auto">
        ...
        {view === 'dashboard' && (
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {[
                { label: 'Viktorinalar', value: quizzes.length, icon: '📚', color: 'purple' },
                { label: 'Jami savollar', value: quizzes.reduce((a, q) => a + q.questions.length, 0), icon: '❓', color: 'cyan' },
                { label: 'Faol o\'yin', value: activeSession && activeSession.status !== 'finished' ? 1 : 0, icon: '🎮', color: 'green' },
                { label: 'Ishtirokchilar', value: activeSession ? Object.keys(activeSession.players).length : 0, icon: '👥', color: 'pink' },
              ].map(stat => (
              ...
              <div className="bg-gradient-to-r from-green-600/20 to-cyan-600/20 border border-green-500/30 rounded-2xl p-5 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-white/50 text-sm mt-1">
                    {Object.keys(activeSession.players).length} ta ishtirokchi • Savol {activeSession.currentQuestionIndex + 1}/{activeSession.quiz.questions.length}
                  </p>
                </div>
            ...
            <div>
              ...
              {quizzes.length === 0 ? (
                <div className="bg-gradient-card rounded-2xl p-12 text-center">
                  ...
                  <p className="text-white/30 text-sm mt-2">Viktorina yarating yoki fayl yuklang</p>
                </div>
              ) : (
                  ...
                  {quizzes.map(quiz => (
                    <div key={quiz.id} className="bg-gradient-card rounded-2xl p-5 hover:border-purple-500/50 transition-all border border-transparent">
                      ...
                      <div className="flex gap-2 mt-4">
                        ...
                        <button onClick={() => { setEditingQuiz(quiz); setView('create-quiz') }} className="px-3 py-2 bg-white/5 hover:bg-white/10 text-white/60 hover:text-white rounded-xl transition-all text-sm">
                          ✏️
                        </button>
                        <button onClick={() => handleDeleteQuiz(quiz.id)} className="px-3 py-2 bg-white/5 hover:bg-red-500/20 text-white/60 hover:text-red-400 rounded-xl transition-all text-sm">
                          🗑️
                        </button>
                      </div>
                    </div>
        ...
        {view === 'import' && (
          <div className="space-y-6">
            ...
            <QuizImporter onImported={async () => { const q = await getQuizzes(); setQuizzes(q) }} />
          </div>
        )}
        ...
        {view === 'lobby' && activeSession && sessionId && (
          <AdminLobby
            ...
            onStart={() => startCountdown(sessionId)}
        ...
        {view === 'monitor' && activeSession && sessionId && (
          <AdminMonitor
            session={activeSession}
            onRevealAnswer={() => revealAnswer(sessionId)}
            onShowLeaderboard={() => showLeaderboard(sessionId)}
            onNextQuestion={() => nextQuestion(sessionId, activeSession.currentQuestionIndex, activeSession.quiz.questions.length)}
